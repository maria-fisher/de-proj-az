...
def generate_data(start_date, end_date):
    ...
    while current_date <= end_date:
        ...
        for station in stations:
            if season == "Winter":
                water_activity = random.uniform(0.9, 0.95)  # Adjust water activity range for winter
            elif season == "Spring":
                water_activity = random.uniform(0.85, 0.92)  # Adjust water activity range for spring
            elif season == "Summer":
                water_activity = random.uniform(0.8, 0.88)  # Adjust water activity range for summer
            else:  # Fall
                water_activity = random.uniform(0.85, 0.92)  # Adjust water activity range for fall
            
            # Append data
            data.append([current_date, station, water_activity, month])
...
# Create DataFrame
df = pd.DataFrame(data, columns=["Timestamp", "Station", "Water Activity", "Month"])
...
# Save to CSV
df.to_csv("food_storage_water_activity.csv", index=False)
